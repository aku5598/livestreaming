...
public class PlayerActivity extends AppCompatActivity {
        ...
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(final Call call, final Response response) throws IOException {
                ...
                try {
                    ...
                    JSONArray results = json.getJSONArray("results");
                    JSONObject latestBroadcast = results.optJSONObject(0);
                    resourceUri1= latestBroadcast.optString("resourceUri");
                } catch (Exception ignored) {}
                final String uri = resourceUri1;
                runOnUiThread(new Runnable() { @Override public void run() {
                    initPlayer1(uri);
                }});
            }
    ...
    void initPlayer(String resourceUri) {
        ...
        mBroadcastPlayer.setSurfaceView(mVideoSurface1);

        mBroadcastPlayer.load();

    }

    void initPlayer1(String resourceUri1) {
        if (resourceUri1 == null) {
            if (mPlayerStatusTextView != null)
                mPlayerStatusTextView.setText("Could not get info about latest broadcast");
            return;
        }
        ...
        if (mBroadcastPlayer != null)
            mBroadcastPlayer.close();
        mBroadcastPlayer = new BroadcastPlayer(this, resourceUri1, APPLICATION_ID, mBroadcastPlayerObserver);
